...
function TestRunner(){
    ...
    let launchTest = (test) => {
        ...
        function onExitEventHandlerWrapper(test) {
            return function (code, signal) {
                ...
                if (code === 0 && test.result.pass === null) {
                    ...
                    reportFileStructure.passed.push(test);
                } else {
                    test.result.pass = false;
                    reportFileStructure.failed.push(test);
        ...
        function onErrorEventHandlerWrapper(test) {
            return function (error) {
                debug(`Worker ${worker.pid} - error event.`, test);
                debug(error);

                test.result.pass = false;
                test.result.messages.push(error);

                reportFileStructure.failed = test;
    ...
    let doReports = () => {
        ...
        console.log(`${reportFileStructure.passed.length} passed tests and ${reportFileStructure.failed.length} failed tests.`);
        for(let i=0; i<this.session.processedTestCount; i++){
            let test = this.testList[i];
            console.log(`   ${test.result.pass ? 'V' : 'X'}   ${test.data.name}`);
        }
        doHTMLReport();
        //this.callback(null, this.session);
    };
    ...
    let doHTMLReport = () => {
        ...
        fs.readFile(path.join(folderName,'/utils/report.html'), 'utf8', (err, res) => {
            ...
            let content = res.replace("</html>", `<script>\nprint(${JSON.stringify(reportFileStructure)});\n</script>\n</html>`);
            fs.writeFile(destination, content, 'utf8', (err) => {
                ...
                debug(`Finished writing report to ${destination}`);
                this.callback();
            });
        });
    };
    ...
    this.start = function(cfg, callback){
        ...
        let testTree = discoverTestFiles(config.testsDir, config);
        this.testList = testTreeToList(testTree);
