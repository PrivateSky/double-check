/*
 ...
 10 whyLog for code reasoning
 11 testResult to log running tests
...
exports.init = function(sf){
    ...
    function createDebugRecord(level, type, message, exception, saveStack, args, pos, data,logOnlyCurrentWhyContext){
        ...
        try {
            if (system_working_with_SwarmCore_library_do_not_touch_please === true) {
    ...
    sf.logger.addCase('throttling', function(message){
        sf.logger.record(createDebugRecord(4, 'throttling', message, null, false));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('warning', function(message){
        sf.logger.record(createDebugRecord(5, 'warning', message,null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('info', function(message){
        sf.logger.record(createDebugRecord(6, 'info', message,null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('debug', function(message){
        sf.logger.record(createDebugRecord(7, 'debug', message,null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('ldebug', function(message){
        sf.logger.record(createDebugRecord(8, 'ldebug', message, null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('udebug', function(message){
        sf.logger.record(createDebugRecord(9, 'udebug', message ,null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase('devel', function(message){
        sf.logger.record(createDebugRecord(10, 'devel', message, null, false, arguments, 0));
    }, [
        {
            'message':'explanation'
        }
    ...
    sf.logger.addCase("logwhy", function(logOnlyCurrentWhyContext){
        sf.logger.record(createDebugRecord(11,'logwhy',undefined,undefined,undefined,undefined,undefined,undefined,logOnlyCurrentWhyContext))
    });

    sf.logger.addCase("recordAssert", function (message, error){
        sf.logger.record(createDebugRecord(12,'assert',message,error,true))
    });
}
