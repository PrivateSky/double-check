...
exports.init = function(sf){
    sf.exceptions.register('assertFail', function(explanation){
        ...
        logger.recordAssertResult(message,err);
        throw err
    });
    ...
    */
    sf.assert.addCheck('fail', function(testName ,func){
        try{
            func();
            logger.recordAssertResult("[Fail] " + testName );
        } catch(err){
            logger.recordAssertResult("[Pass] " + testName );
        }
    })
    ...
    sf.assert.addCheck('pass', function(testName ,func){
        try{
            ...
            logger.recordAssertResult("[Fail] " + testName  ,  err.stack);
        }
    });
    ...
    sf.assert.addCheck('callback', function(testName ,func, timeout){
        ...
        function callback(){
            if(!passed){
                ...
                logger.recordAssertResult("[Fail (multiple calls)] " + testName );
            }
        }
        try{
            func(callback);
        } catch(err){
            logger.recordAssertResult("[Fail] " + testName  ,  err);
        }

        function SuccessTest(force){
            if(!passed){
                logger.recordAssertResult("[Fail Timeout] " + testName );
            }
        }
    ...
    sf.assert.addCheck('steps', function(testName , arr, timeout){
        ...
        function next(){
            ...
            try{
                ...
                logger.recordAssertResult("[Fail] " + testName  + " [at step " + currentStep + "]"  , err);
            }
        }

        function SuccessTest(force){
            if(!passed){
                logger.recordAssertResult("[Fail Timeout] " + testName  + " [at step " + currentStep + "]" );
            }
        }
    ...
    sf.assert.addCheck('begin', function(message, timeOut){
        logger.recordAssertResult(message);
