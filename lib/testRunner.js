...
var defaultConfig = {
    ...
    testsDir: __dirname,                // path to the root tests location
    reports: {
        basePath: __dirname,            // path where the reports will be saved
...
console.log(this.testTree);
        ...
        __doTestReports: function() {
            ...
            let fileName = `${this.config.reports.prefix}${timestamp}${this.config.reports.ext}`;
            let filePath = path.join(this.config.reports.basePath, fileName);
            writeReports.__saveReportToFile(reportFileStructure, filePath);

            const htmlFileName = `${this.config.reports.prefix}${timestamp}.html`;
        ...
        __saveHtmlReportToFile: function (reportFileStructure, destination, timestamp) {
            fs.readFile('./lib/utils/reportTemplate.html', 'utf8', (err, res) => {
                if (err) {
