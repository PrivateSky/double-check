...
exports.init = function(sf){
    sf.testRunner = $$.flow.describe("testRunner", {
        ...
        __launchTest: function(test) {
            ...
            var worker = forker.fork(test.data.path, [], {'cwd': cwd, 'env': env, 'execArgv': execArgv, stdio: ['inherit', "pipe", 'inherit', 'ipc'], silent:false });
            ...
            worker.timerVar = setTimeout(function(){
                if(!worker.terminated){
                    console.log(`worker pid [${worker.pid}] - timeout event`, test);
        ...
        __saveHtmlReportToFile: function (reportFileStructure, destination, timestamp) {
            ...
            if(folderName[0] == "/"){
                //see https://github.com/browserify/browserify/issues/1723
