/**
 * TODO This check should be removed, right?
 */
...
exports.init = function(sf){
    sf.testRunner = $$.flow.describe("testRunner", {
        /**
        ...
        __launchTest: function(test) {
            ...
            var worker = forker.fork(test.data.path, [], {'cwd': cwd, 'env': env, 'execArgv': execArgv, stdio: ['inherit', "pipe", 'inherit', 'ipc'], silent:true });
            ...
            function onExitEventHandlerWrapper(test) {
                return function(code, signal) {
                    ...
                    if(code!==0 && typeof test.result.pass === 'undefined'){
                        test.result.pass = false;
                        test.result.messages[1][0] = "Process finished with errors: "+code+" "+signal;
                    }else{
                        console.log(`Worker ${worker.pid} - exit event. Code ${code}, signal ${signal} ${test.result.pass}`);
                    }
