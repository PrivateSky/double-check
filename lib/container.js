...
function Container(sf){
    ...
    var callbacks = {};     //callback that shoudl be called for each dependency declaration
    var depsCounter = {};   //count dependencies
    var reversedTree = {};  //reversed dependencies, opposite of immediate obect
    ...
    function insertDependencyinRT(nodeName, dependencies){
     ...
     /*
        discover nodes waiting a single dependency, and that dependency is nodeName
     */
    function discoverWaitingNodes(nodeName){
        var ret = [];
        for( var v in depsCounter){
            if(depsCounter[v] == 1){
                if(immediate[v].indexOf(nodeName) != -1){
                    ret.push(v);
                }
            }
        }
    ...
    this.resolve    = function(name, value){
        ...
        /*var upNodes = discoverWaitingNodes(name);
        upNodes.forEach(function(node){
            callForThing(node, false);
            resetCounter(name)
        })*/
    }
    ...
    /*
        Declare that a name is again not ready
     */
